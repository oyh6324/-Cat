...
public class ClothesItemData 
{ 
    public ClothesItemData(string _name, string _setname, string _ex, string _type, int _level, int _count, int _star, int _str, int _def,int _agi,
...
public class MoneyItemData
{
    public MoneyItemData(string _name, int _count)
    {
...
{
    public CharacterData(string _name,string _helmet,string _top, string _bottoms, string _weapon,string _setname,int _level, int _str, int _def, int _agi, int _hp,
        int _itemstr, int _itemdef, int _itemagi, int _allstr, int _alldef, int _allagi, int _setstr, int _setdef, int _setagi,int _stage,int _exp, int _totalexp, decimal _crip, decimal _itemcrip,
        decimal _allcrip,decimal _setcrip, decimal _itemspeed)
    {
...
{
    public AchievementData(string _name, string _rewardname, string _rewardname2, int _level, int _maxlevel, int _progressvalue, int _rewardcount, int _rewradcount2, int _value)
    {
...
{
    public MonsterCollectionData(string _name, string _realname, string _habitat, string _kind, int _stage,bool _isreward)
    {
...
{
    public static List<WeaponItemData> allWeaponItemList;
    public static List<ClothesItemData> allClothesItemList;
    public static List<CharacterData> characterDatasList;
    public static List<AchievementData> achievementDataList;
    public static List<MonsterCollectionData> monsterCollectionDataList;
    //0: anchovy 1:pearl 2:key 3:anchovyboxClothes 4:pearlboxClothes 5:anchovyboxWeapon 6:pearlboxWeapon 
    public static List<MoneyItemData> moneyItemList;
    ...
    {
        allClothesItemList = new List<ClothesItemData>(); //의상
        allWeaponItemList = new List<WeaponItemData>(); //무기
        characterDatasList = new List<CharacterData>(); //캐릭터 스탯
        moneyItemList = new List<MoneyItemData>(); //돈
        achievementDataList = new List<AchievementData>(); //업적
        monsterCollectionDataList = new List<MonsterCollectionData>(); //컬렉션

        //리소스 파일의 텍스트 형식의 의상 데이터 부르기
        TextAsset clothesDataText = Resources.Load("ClothesDataText", typeof(TextAsset)) as TextAsset;
        ...
        for (int i = 0; i < lineClothes.Length; i++)
        {
            ...
            bool isnew = false;
            allClothesItemList.Add(new ClothesItemData(row[0], row[1], row[2], row[3], toInt[0], toInt[1], toInt[2], toInt[3], toInt[4], toInt[5], toInt[6], toInt[7], toInt[8], toInt[9], toDecimal, isnew));
        }
        //리소스 파일의 텍스트 형식의 무기 데이터 부르기
        TextAsset weaponDataText = Resources.Load("WeaponDataText", typeof(TextAsset)) as TextAsset;
        ...
        for (int i = 0; i < lineWeapon.Length; i++)
        {
            ...
            bool isNew=false;
            allWeaponItemList.Add(new WeaponItemData(row[0], row[1], toInt[0], toInt[1], toInt[2], toInt[3], toDecimal[0], toDecimal[1],isNew));
        ...
        for (int i = 0; i < lineAchievement.Length; i++)
        {
            ...
            achievementDataList.Add(new AchievementData(row[0], row[1], row[2], toInt[0], toInt[1], toInt[2], toInt[3], toInt[4],toInt[5]));
        ...
        for (int i = 0; i < linecollection.Length; i++)
        {
            ...
            bool isreward = false;
            monsterCollectionDataList.Add(new MonsterCollectionData(row[0],row[1],row[2],row[3], stage, isreward));
        }
        //캐릭터 데이터 추가
        characterDatasList.Add(new CharacterData("User", "", "", "", "", "", 1, 7, 3, 5, 100, 0, 0, 0, 7, 3, 5, 0, 0, 0, 0, 0,600,5, 0, 5, 0, 0));
        //돈, 무료이용권 데이터 추가
        moneyItemList.Add(new MoneyItemData("멸치", 1000000));
        moneyItemList.Add(new MoneyItemData("진주", 10000));
        moneyItemList.Add(new MoneyItemData("열쇠", 5));
        moneyItemList.Add(new MoneyItemData("멸치보석함의상", 1));
        moneyItemList.Add(new MoneyItemData("진주보석함의상", 10));
        moneyItemList.Add(new MoneyItemData("멸치보석함무기", 10));
        moneyItemList.Add(new MoneyItemData("진주보석함무기", 1));
    ...
    void LoadAllData() //json 불러오기
    {
        ...
        allClothesItemList = JsonConvert.DeserializeObject<List<ClothesItemData>>(jdataC);

        //weaponItemData
        string jdataW = File.ReadAllText(Application.persistentDataPath + "/WeaponItem.json"); 
        allWeaponItemList = JsonConvert.DeserializeObject<List<WeaponItemData>>(jdataW);

        //moneyItemData
        string jdataM= File.ReadAllText(Application.persistentDataPath + "/MoneyItem.json");
        moneyItemList = JsonConvert.DeserializeObject<List<MoneyItemData>>(jdataM);

        //characterData
        string jdataCh = File.ReadAllText(Application.persistentDataPath + "/CharacterData.json");
        characterDatasList = JsonConvert.DeserializeObject<List<CharacterData>>(jdataCh);

        //achievementData
        string jdataA = File.ReadAllText(Application.persistentDataPath + "/AchievementData.json");
        achievementDataList = JsonConvert.DeserializeObject<List<AchievementData>>(jdataA);

        //monsterCollectionData
        string jdataMC = File.ReadAllText(Application.persistentDataPath + "/MonsterCollectionData.json");
        monsterCollectionDataList = JsonConvert.DeserializeObject<List<MonsterCollectionData>>(jdataMC);
    }
}
